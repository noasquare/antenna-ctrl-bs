...
tag app
	...
	def wsopen url
		...
		# 进行进来的信号做初始值设定
			...
			jihua2 = devdata[0].Devices[0].StatusList[0].params.Polar2Name
			# 判断服务器发来的用户检查命令
	...
	def render()
		...
		<self>
			...
			<div.main[of:hidden]>
				...
				<tianxian$txjm[w:100%] route='/txst/:id' data=devdata ws=socket antindex=antindex islogin=isLogin jihua1=jihua1 jihua2=jihua2> # 这里的index是为了区分是哪个天线的数据，但是如果这样，就会导致纯通过route不能获取最新的数据了。解决方案就是，点击要的index传进去，然后再component里面直接做，不转手。
